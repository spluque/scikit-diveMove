...
class IMUcalibrate(IMUBase):
    ...
    obs_l : list
        List of :class:`Dataset`, where each one is a subset of the input
        :class:`Dataset`, bounded by the slices in the `periods` attribute.
    models_l : list
    ...
    def plot_standardized(self, var, use_axis_order=True, units_label=None,
        ...
        axs : array_like, optional
            Array of Axes instances to plot in.
        **kwargs : optional keyword arguments
            Arguments passed to :func:`pyplot.subplots` (e.g. ``figsize``).

        Returns
        -------
        tuple, (3, N)
            Pyplot Figure and 2 (IMU signals and temperature) Axes instances.
