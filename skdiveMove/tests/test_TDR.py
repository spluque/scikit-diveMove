...
class TestTDR(ut.TestCase):
    ...
    def test_detect_wet(self):
        ...
        phases_uniq_ids = wet_act_phases.iloc[:, 0].unique()
        wet_act_times = self.tdrX.get_wet_activity("times_begend")
        self.assertIsInstance(wet_act_times, DataFrame)
        self.assertEqual(wet_act_times.ndim, 2)
        self.assertEqual(phases_uniq_ids.size, wet_act_times.shape[0])
