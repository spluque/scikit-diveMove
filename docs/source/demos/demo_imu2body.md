...
# If data are in NetCDF and CSV files, one can take advantage of the class
# method `IMU2Body.from_csv_nc` to instantiate directly from the file
# names:
...
{py:func}`~scipy.signal.savgol_filter`) job:
...
{py:meth}`imutools.IMU2Body.get_orientation`.
...
anim_file = "../.static/video/gert_imu_{}.mp4".format(idx_title)
...
anim_file = "../.static/video/gert_body_{}.mp4".format(idx_title)
...
Display resulting transformation:
