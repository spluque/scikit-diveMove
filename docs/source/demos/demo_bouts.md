...
Here is a brief demo on bout analysis with the {py:mod}`bouts` module for
...
Create a {py:class}`~skdiveMove.TDR` object to easily calculate the
...
{py:mod}`skdiveMove` provides the {py:class}`bouts.BoutsNLS` class for
fitting non-linear least squares (NLS) models to a modified histogram of a
given variable.
...
print("Coefficients:\n{}\n".format(coefs2))
print("Covariance between parameters:\n{}\n".format(pcov2))
...
Label bouts based on BEC from the last MLE model.  Note that
{py:class}`~pandas.Timedelta` type needs to be converted to total seconds to
allow comparison with BEC.
