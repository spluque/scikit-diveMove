...
(demo_simulbouts-label)=
# Simulating bouts

This follows the simulation of mixed Poisson distributions in [Luque &
Guinet (2007)](https://doi.org/10.1163/156853907782418213), and the
comparison of models for characterizing such distributions.
...
{math}`\lambda_s=0.005`, we use the
{py:func}`~skdiveMove.bouts.random_mixexp` function to generate samples.

Define the true values described above, grouping the parameters into a
{py:class}`~pandas.Series` to simplify further operations.
